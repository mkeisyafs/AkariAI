...
## 2026-05-13 — Task T29 (structured logger + telemetry)

- `safeLog()` from `src/utils/encryption.js` (T1) only shape-redacts Discord-token-shaped strings. It doesn't catch API keys or generic secrets, so the logger scrubs by both **key name** (`token`, `newToken`, `apiKey`, `newApiKey`, `aiApiKey`, `encryptedToken`, `encryptedApiKey`, `authorization`, `cookie`, `password`) **and** string shape.
- Prior to T29 each module had its own `logEvent()` helper with nearly identical `try { console.log(JSON.stringify({...})); } catch {}` bodies. Centralizing in `logger.js` dropped those helpers to one-liner `logger.info(event, ctx)` delegates where they still existed (`botManager.logEvent`, `slashCommandDeployer.logEvent`) — kept as thin aliases to minimize diff risk at call sites.
- `bot.status.change` is emitted via a `setStatus(handle, next)` helper in `botManager.js` — centralizes the mutation and guarantees the `from`/`to` fields are always correct. Replaces every `handle.status = 'X'` assignment inside lifecycle functions.
- `bot.connect.success` is emitted **inside the `ClientReady` handler** (not after `client.login()` resolves). Reason: login() resolves when the WebSocket identify is sent, which is before the READY event fires. Only after READY does the bot have a real discordBotUserId — the spec requires `{ botId, userId }`, so READY is the correct trigger.
- `bot.connect.failed` is emitted at the end of **every** failure branch in `connectInsideLock` — token lookup failure, TOKEN_INVALID login rejection, and generic login error. Paired with the narrower-event (`bot.connect.token_lookup_failed`, `bot.token.invalid`, `bot.login.failed`) so ops dashboards can either roll up via `bot.connect.failed` or drill in by sub-event.
- Scrub function recurses into arrays and nested objects up to depth 6; beyond that returns `'[depth]'` sentinel. Protects against circular-ref infinite loops that would crash the logger (the previous per-module helpers crashed on circular ctx — discovered when attempting to log `err` objects that carry `cause`/`response` cycles).
- `Error` instances are detected in scrub and converted to `{ error, code }`. Bare `JSON.stringify(err)` loses `message` because it lives on the prototype. Every caller that passes `err` now gets both fields without having to destructure manually.
